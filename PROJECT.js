...
function createStadiumLight(x, y, z) {
    const light = new THREE.PointLight(0xfff0dd, 0.0, 50); // Partono a intensità 0
...
function updateDayNightCycle(time) {
    // time: da 0.0 a 24.0. (6 = Alba, 12 = Mezzogiorno, 18 = Tramonto, 0/24 = Notte Fondente)
    const angle = ((time - 6) / 24) * Math.PI * 2; 
    
    // Orbita del Sole/Luna
    sunLight.position.x = -Math.cos(angle) * 50; 
    sunLight.position.y = Math.abs(Math.sin(angle)) * 50 + 10; // Si assicura che la luce provenga sempre dall'alto
    ...
    // Interpolazione Fluida di Colori e Intensità
    if (time >= 5 && time < 8) { // Alba (5 - 8)
        ...
        sunLight.intensity = 0.5 - (mix * 0.3);
        hemiLight.intensity = 0.3 - (mix * 0.2);
    } else { // Notte (21 - 5)
    ...
    if (time >= 19.5 || time <= 5.5) {
        ...
        lampIntensity = (time - 18.5) * 20.0; // Si accendono gradualmente al tramonto (funzione crescente allo scorrere del tempo)
    } else if (time > 5.5 && time < 6.5) {
        lampIntensity = (1 - (time - 5.5)) * 20.0; // Si spengono gradualmente all'alba (funzione decrescente allo scorrere del tempo)
    }
